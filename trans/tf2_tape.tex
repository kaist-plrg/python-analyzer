...
\begin{longtable}{l}
  ...
  \inden\inden
  ([\nidsubs{r} \oassign \nexprsubs{1} \sparen{\nexprsubs{11} ... \nexprsubs{1n} ~ \op{(\nidsubs{1} \oassign)} \nexprsubs{21} ... \op{(\nidsubs{k} \oassign)} \nexprsubs{2k}} \optypcomm],\\
  \inden\inden\inden\smodenv[\optimizer $\mapsto$ \nidsubs{r}]) \\
...
\noindent
\begin{tabular}{l}
  \tstmt{\mul{\nexprsubs{1}} \oassign \nexprsubs{2} \optypcomm}{\smodenv} = \\
  % CUDA visible device
  \inden\comment{CUDA Visible Devices} \\
  \inden \ktif ~ \smodenv(\os) ~ \kteq ~ \nidsubs{os} ~ \ktand ~ \nexprsubs{0} ~ \kteq ~ \nidsubs{os}{\tt.environ} \lparen{\tt `CUDA\_VISIBLE\_DEVICES'} ~ \ktthen\\
  \inden\indent ([], \smodenv) \\
...
\begin{tabular}{l}
  ...
  % adjust steps
  \inden \comment{Adjust Steps} \\
  \inden \ktif ~ \smodenv(\config) ~ \kteq ~ \nidsubs{c} ~ \ktand ~ (\nexprsubs{2} ~ \kteq ~ {\tt range(\nidsubs{c}.iterations\_per\_epoch)} ~ \ktor \\
  \inden\inden \nexprsubs{2} ~ \kteq ~ {\tt tqdm(range(\nidsubs{c}.iterations\_per\_epoch))}) ~ \ktthen \\
  \inden\inden\inden ([\optypcomm ~ \kfor ~ \nexprsubs{1} ~ \kin ~ {\tt range(\nidsubs{c}.iterations\_per\_epoch // hvd.size())} ~ \kcolon \\
  \inden\inden\inden\inden \tsstmt{\mul{\nstmtsubs{1}}}{\smodenv}\fst ~ \op{(\kelse ~ \kcolon ~ \tsstmt{\mul{\nstmtsubs{2}}}{\smodenv}\fst)}], \smodenv)\\
  \inden\ktelse ~ ([\optypcomm ~ \kfor ~ \nexprsubs{1} ~ \kin ~ \texpr{\nexprsubs{2}}{\smodenv} ~ \kcolon \\
...
\begin{tabular}{l}
  ...
  \inden \ktif ~ \smodenv(\config) ~ \kteq ~ \nidsubs{c} ~ \ktand ~ (\nexprsubs{2} ~ \kteq ~ {\tt range(\nidsubs{c}.iterations\_per\_epoch)} ~ \ktor \\
  \inden\inden \nexprsubs{2} ~ \kteq ~ {\tt tqdm(range(\nidsubs{c}.iterations\_per\_epoch))}) ~ \ktthen \\
  \inden\inden\inden ([\optypcomm ~ \kasync ~ \kfor ~ \nexprsubs{1} ~ \kin ~ {\tt range(\nidsubs{c}.iterations\_per\_epoch // hvd.size())} ~ \kcolon \\
  \inden\inden\inden\inden \tsstmt{\mul{\nstmtsubs{1}}}{\smodenv}\fst ~ \op{(\kelse ~ \kcolon ~ \tsstmt{\mul{\nstmtsubs{2}}}{\smodenv}\fst)}], \smodenv)\\
  \inden\ktelse ~ ([\optypcomm ~ \kasync ~ \kfor ~ \nexprsubs{1} ~ \kin ~ \texpr{\nexprsubs{2}}{\smodenv} ~ \kcolon \\
  \inden \inden \tsstmt{\mul{\nstmtsubs{1}}}{\smodenv}\fst ~ \op{(\kelse ~ \kcolon ~ \tsstmt{\mul{\nstmtsubs{2}}}{\smodenv}\fst)}], \smodenv)\\
\end{tabular}\\\vpar
