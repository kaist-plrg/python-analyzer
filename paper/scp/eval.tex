...
\begin{inred}
Note that our evaluation targes consists of models written purely in
...
We discuss on this limitation and possible solution
...
% instance.
\begin{inred}
We argue that this is an unconventional case as
other 11 GradientTape models uses the {\tt Model} API to construct the model.
As we aim to identify API pattern commonly used in TensorFlow DL models,
we report limitation of our approach, where model codes written in unconventional
API usage cannot be correctly transformed.
\end{inred}

\begin{inred}
In conclusion, the result of RQ1 experiment shows that our code transformation
approach correctly transforms most of TensorFlow DL models.
Our tool fails in one case where the code uses the training APIs in
unconventional way, which is a limitation of our approach.
\end{inred}


\subsection{RQ2: \begin{inred}Efficiency\end{inred}of the automatically distributed training}
...
The result if the RQ2 experiment show that the distributed training of the 
...
We do not report the distributed time of Stacked-LSTM-ColorBot case
...
In six out of 13 models, distributed training reaches higher loss compared to
the original model.
...
the loss doses not meaningfully decrease and remains unchanged, 
...
resulting distributed model trains faster or has the same accuracy.
\end{inred}
...
\begin{inred}
We argue that tuning the training hyperparameters can addiitonally improve
