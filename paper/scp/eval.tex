...
other 11 GradientTape models use the {\tt Model} API to construct the model.
As we aim to identify API patterns commonly used in TensorFlow DL models,
we report the limitation of our approach, where model codes are written in unconventional
API usage cannot be correctly transformed.
\end{inred}

\begin{inred}
In conclusion, the result of the RQ1 experiment shows that our code transformation
approach correctly transforms most TensorFlow DL models.
