...
trans_S(`import {aliases}`, ctx):
  ...
  if diff(ctx_1, ctx) == ["tensorflow" -> `{id}`]:
    ...
    return (`import {aliases}`, ctx_1)\end{lstlisting}
...
to subsequent transform function calls and utilize the information from outside
their input AST.

Figure \ref{fig:trans:ex01:code} illustrate the pseudocode of an example
...
In the pseudocode, we use back-quoted notation {\tt \textasciigrave import
\{aliases\}\textasciigrave} to represent a Python AST object;
the brace-surrounded expression represents a child AST.
Line 2 creates a new context object by calling the transformation function on
the child AST, {\tt \textasciigrave\{aliases\}\textasciigrave}.
Then, line 3 computes the difference between the original context object {\tt ctx}
and the new context object {\tt ctx\_1}.
If the new context additionally stores the {\tt "tensorflow"} entry, it infers
that the import statement imports the TensorFlow library.
In this case, the transform function places a new import statement, {\tt import
horovod.tensorflow as hvd}, right after the TensorFlow import statement and
returns the modified AST. 
Otherwise, the function leaves the input AST unchanged.
