...
\noindent
% TensorFlow offers multiple APIs for defining the model structure and the
% training process. 
...
Instead of using the {\tt Session} object, the MonitoredSession pattern
codes use the {\tt MonitoredSession} object that provides hook methods.
...
Then, the {\tt Optimizer} class instance calls the method {\tt
apply\_gradients} to update the model parameters.
...
The {\tt fit} method is then invoked to automatically train the model with a
given dataset.
...
To handle these scenarios, \tapi~performs a simple static analysis based on  a
