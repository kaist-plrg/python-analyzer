...
trans_S(`{id_r} = {expr_1}({expr_11}, ..., {expr_1n})`, ctx): 
  if isSubclass(`{expr_1}`, `keras.optimizer.Optimizer):
    for i in 1...n:
      if `{expr_1i}` == `learning_rate = {lr}`:
	    let new_expr = `{id_r} = {expr_1}({expr_11}, .., {expr_1i} * hvd.size(), .., {expr_1n})`
		let wrap_expr = `{id_r} = hvd.DistributedOptimizer({id_r})`
