...
In line 5, a model is constructed as an instance of the {\tt ResNet} class.
Finally, in line 6, the {\tt fit} method call trains the model with given
dataset.
...
When the the identified pattern is either Multiple or Unknown, our tool fails
to transform the model in the automated way and terminates. 
Note that the transformation failures are rare in that our evaluation shows
that \tapi~identifies only one of the 16 target TensorFlow models as either
Multiple or Unknown.
