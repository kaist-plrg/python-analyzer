...
Although the example shown is relatively short,
the general process applies to any example training codes.
The transform function is defined in terms of
pattern matching agains the input,
pattern guard to check the content of input AST,
and output representing the transform result.

We describe the structure transform function definition for each language
construct, and present some important parts of the definition
to illustrate the automated code transformation for distributed training.
The full definition of the transformation rule can be found
