...
If the difference contains the new entry for the string {\tt "tensor\_flow"},
the transform function returns a statement list containing the original
statement and a new statement {\tt import horovod.tensorflow as hvd} to add the
import statement for the {\tt horovod} module after the original statement as
shown in Figure~\ref{fig:trans:ex01}(b).  
Otherwise, the function returns a statement list containing only the original
statement remaining unchanged.

In the next subsection, we briefly present key code transform rules for
