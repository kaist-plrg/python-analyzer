...
This should achieve a test error of 0.7%. Please keep this model as simple andlinear as possible, it is meant as a tutorial for simple convolutional models.Run with --self_test on the command line to execute a short self-test."""
...
def main(_, ):
  ...
  loss += 5.0E-4 * regularizers
  batch = tf.Variable(0, dtype=tf.int64, trainable=False, )
  learning_rate = tf.train.exponential_decay(0.01 * hvd.size(), batch * BATCH_SIZE, train_size, 0.95, staircase=True, )
  optimizer = tf.train.MomentumOptimizer(learning_rate, 0.9, )
  optimizer = hvd.DistributedOptimizer(optimizer, ).minimize(loss, global_step=batch, )
  ...
  with tf.Session(config=config, ) as sess:
    ...
    hvd.broadcast_global_variables(root_rank=0, ).run()
    if hvd.rank() == 0:
      print("Initialized!", )
