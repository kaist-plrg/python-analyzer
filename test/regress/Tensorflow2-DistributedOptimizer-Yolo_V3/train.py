...
flags.DEFINE_enum("mode", "fit", ["fit", "eager_fit"], "fit: model.fit, eager_fit: model.fit(run_eagerly=True)", )
...
def main(_argv, ):
  ...
  if FLAGS.transfer == "none":
    ...
    if FLAGS.transfer == "fine_tune":
      ...
      freeze_all(model, )
  optimizer = tf.keras.optimizers.Adam(learning_rate=FLAGS.learning_rate * hvd.size(), )
  ...
  callbacks = [ReduceLROnPlateau(verbose=1, ), EarlyStopping(patience=3, verbose=1, ), ModelCheckpoint("checkpoints/yolov3_train_{epoch}.tf", verbose=1, save_weights_only=True, ), TensorBoard(log_dir="logs", )]
