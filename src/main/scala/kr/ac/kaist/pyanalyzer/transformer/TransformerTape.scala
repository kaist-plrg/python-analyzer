...
object TransformerTape extends Transformer {
  ...
  def apply(ast: Module): Module = ast match {
    case m @ Module(body, tyIg, name) =>
      Module(transform(body)(Env())._1, tyIg, name)
  }

  /////////////////////////////////////////
  // transformer for statements
  /////////////////////////////////////////
  override def transform(stmts: List[Stmt])(implicit env: Env): (List[Stmt], Env) = 
  ...
  /////////////////////////////////////////
  override def transform(comp: Comprehension)(implicit env: Env): Comprehension = 
    comp match {
  ...
  override def transform(handler: ExcHandler)(implicit env: Env): ExcHandler = 
    handler match {
      case ExcHandler(except, idOpt, body) =>
        ExcHandler(except, idOpt, transform(body)._1)
    }

  override def transform(al: List[Alias])(implicit env: Env): Env = 
  ...
  override def transformWithList(wl: List[WithItem])(implicit env: Env): (List[WithItem], Env) = 
  ...
  override def transform(item: DictItem)(implicit env: Env): DictItem = item match {
    case KVPair(k, v) => KVPair(k, transform(v))
    case DoubleStarred(e) => DoubleStarred(e)
  }

  override def transform(mc: MatchCase)(implicit env: Env): MatchCase = mc match {
    case MatchCase(pat, cond, body) =>
      MatchCase(transform(pat), cond.map(transform), transform(body)._1)
  }

  override def transform(pat: Pattern)(implicit env: Env): Pattern = pat match {
  ...
  // helper functions
  /////////////////////////////////////////
  override def parseStmts(code: String): List[Stmt] = {
    TokenListParser(tokenizeText(code)).body
  }

  override def parseExpr(str: String): Expr = {
  ...
  override def newId: Id = Id("id_new")

  override def findKwarg(lk: List[Kwarg], str: String): Option[NormalKwarg] =
    (lk.find {
      case NormalKwarg(Id(x), _) if x == str => true
      case _ => false
    }).asInstanceOf[Option[NormalKwarg]]

  override def replaceElement[T](lk: List[T], from: T, to: T): List[T] = {
